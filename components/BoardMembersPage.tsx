...
const TOP_BOARD: Member[] = [
  { name: "Michele Russo", role: "PRESIDENTE", image: PLACEHOLDER_IMAGE },
  { name: "Luciano Lombardo", role: "VICE PRESIDENTE", image: PLACEHOLDER_IMAGE },
];

const MID_BOARD: Member[] = [
  { name: "Guido Guglielmaci", role: "SEGRETARIO", image: PLACEHOLDER_IMAGE },
  { name: "Mirko Lombardo", role: "REFERENTE COMUNICAZIONI", image: PLACEHOLDER_IMAGE },
  { name: "Salvatore Vullo", role: "TESORIERE", image: PLACEHOLDER_IMAGE },
];

const BOTTOM_BOARD: Member[] = [
  { name: "Stefania Sicurezza", role: "CONSIGLIERE", image: PLACEHOLDER_IMAGE },
  { name: "Luca Andolina", role: "CONSIGLIERE", image: PLACEHOLDER_IMAGE },
...
const MemberCard: React.FC<{ member: Member; size?: 'lg' | 'md' | 'sm' }> = ({ member, size = 'md' }) => {
  ...
  return (
    <div className="flex flex-col items-center text-center group">
      <div className={`relative ${sizeClasses[size]} rounded-full border-4 border-white overflow-hidden shadow-2xl mb-4 transition-transform duration-500 group-hover:scale-110 bg-gray-200 flex items-center justify-center`}>
         <img 
            src={member.image} 
            alt={member.name} 
            className="w-full h-full object-cover grayscale opacity-80"
         />
         {/* Overlay with icon if we want a more "stylized" silhouette look */}
         <div className="absolute inset-0 bg-black/10 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
            <User className="text-white w-1/3 h-1/3" />
         </div>
      </div>
  ...
  return (
    ...
    >
      ...
      <div className="container mx-auto px-4 relative z-10 flex flex-col items-center">
        ...
        {/* Title - Updated text and reduced size slightly as requested */}
        ...
        <h2 className="font-display font-bold text-2xl md:text-3xl text-white uppercase mb-12 tracking-wide shadow-black drop-shadow-sm">
